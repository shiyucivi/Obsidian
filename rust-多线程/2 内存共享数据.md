...
### 5 多线程提供内部可变性的锁
#### 5.1 mutex
Mutex是最常见的用于在线程间分享数据的类型，同一时刻只允许存在一个对数据的访问。如果访问时Mutex已经被其他线程占用，那么当前线程会阻塞直到Mutex被其他线程释放。
...
#### 5.3 Mutex和RwLock的选择
RwLock的安全性是操作系统提供的，实现原理要比`Mutex`复杂的多，因此RwLock的性能比不上原生实现的`Mutex`。但高并发读取下Mutex很容易出现解锁失败的情况，因此只有需要高并发读取的情况下使用RwLock，其他情况优先使用Mutex。
...
```
### 8 `Cow`写时克隆
`Cow`是Copy on clone的缩写。它同时拥有借用数据和拥有数据的功能，仅在必须修改数据时才进行内存分配和复制。
`Cow<'a, B>`是一个枚举类型包含两种状态：
- **`Borrowed(&'a B)`**：持有数据的**引用**。此时没有发生内存分配，零开销。
- **`Owned(<B as ToOwned>::Owned)`**：持有数据的**所有权**（通常是 `B` 的拥有所有权的版本，如 `String` 或 `Vec`）。此时数据是独立堆分配的。
初始状态下，`Cow`处于`Borrowed`，读取时就像一个普通的引用一样。但是需要写入（`to_mut`方法）时，`Cow`会自动执行`clone`进入`Owned`状态，并返回可变引用。
如果一个数据结构，大部分时间是只读的，只有偶尔需要修改，且希望 API 既能处理引用又能处理所有权，那么 `Cow` 是最佳选择。
### 9 线程挂起与线程继续
条件变量`Condvar`经常和 `Mutex` 一起使用，可以让线程挂起，并等待其他线程通知结束挂起状态。
```rust
use std::thread;
...
```
这个例子中，主线程先获取到锁，然后while循环判断锁的数据为false，`Condvar.wait`接收一个MutexGuard作为参数，会释放锁并让线程挂起从而避免一直处于while循环。
从线程获取到锁，然后将锁的数据置为true，并`notify_one()`通知其他线程。主线程被唤醒，重新进入while循环。