通道是单一所有权的。即一块数据内存只能由一个线程来管理。单个数据在多线程中移动也必须转移所有权。
...
## 2 `Arc<T>`原子引用计数
`Arc<T>`是专为多线程设计的引用计数智能指针。损失了一部分性能换取线程安全性。用法类似`Rc`，需要对&self进行clone再使用。
`Arc`的引用计数是原子操作，而`Rc`不是，如果多线程同时修改`Rc`的计数就会引发数据竞争从而崩溃，因此`Rc`不能用于多线程。
**`Arc<T>`的`T`必须满足`Send`+`Sync`，通常只用于拥有所有权的数据以及静态生命周期数据，不用于包裹引用类型&T**，因为无法保证引用的生命周期在线程间存活。
```rust
use std::sync::Mutex;
use std::thread;
use std::sync::Arc;
...
```
`Arc`不提供可变引用，通常配合`Mutex`或者`RwLock`等提供内部可变性的锁使用。
## 3 Send与Sync
