stream就用于在异步任务之间传递的数据流，本质上就是Futures的集合。是`Iterator`的异步版本。Iterator有一个同步的`next`方法用于逐个地遍历每个元素。stream也有一些异步方法来逐个遍历。
```rust
trpl::run(async {
...
```
## 2 迭代和并发
与迭代器类似，可以对Stream使用map、filter等迭代器进行处理。可以使用while let循环，但是无法使用for循环。
例如异步计算一个输出类型为`i32`的Stream之和。
```rust
async fn sum_with_next(mut stream: Pin<&mut dyn Stream<Item = i32>>) -> i32 { 
	use futures::stream::StreamExt; //因为stream的trait
	let mut sum = 0; 
