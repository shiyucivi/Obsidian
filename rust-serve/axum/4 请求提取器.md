...
### 5 Header
#### 5.1 HeaderMap
通过`http::HeaderMap`获取一个Map。Map的Key可以通过字符串传值，也可以在`axum::http::header`中找到各类型。
```rust
use axum::{
	http::{HeaderMap}
}
...
```
#### 5.2 `axum_extra::TypedHeader`
`axum_extra::TypedHeader` 的作用是**以类型安全且便捷的方式提取和验证 HTTP 请求头**。不需要进行复杂的错误处理。
```rust
...
```
如果请求头不存在则直接返回错误响应。如果不想直接报错可以使用`Option<TypedHeader<T>>`。
### 6 Method
`http::Method`可以提取请求的方法：
```rust
use axum::http;
