...
例如str类型在程序当中是不可直接使用的，只能通过&str胖指针来获取。str存储在栈上面（使用`Vec<u8>`实现），是String和&str的底层类型（String和&str是指向str的胖指针）。
绝大多数类型以及默认的泛型都满足Sized trait。但是动态大小类型不满足。如果一个类型是动态大小类型，那么必须加上指针或者引用&符号或者Box包裹才能使用。
```rust
fn generic<T: ?Sized>(t: &T) {}
```
如果想把一个String这种胖指针转化为`box<str>`，可以使用String的`into_boxed_str()`方法，但这种类型用处较少。
常见的DST类型还有trait对象。函数的trait object参数必须是`Box<dyn trait>`或者`&dyn trait`。
Rust中不允许我们直接使用DST类型（因为栈上的数据要求必须是固定大小），因此所有所有DST都要转化为引用或者指针类型才能使用。
## 4 高级函数和闭包
