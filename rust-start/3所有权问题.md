...
在rust中，每运行到一个作用域，会创建一个栈帧stackframe，所有变量都会创建一个栈stack推入在栈帧中。这些变量有的是整形、字符、元组等类型的数据，可以直接放在stack中。如果是Vec、String等需要保存在heap中的数据，则会创建一个指针（瘦指针或胖指针）放在stack中，指针指向heap中的内存地址。
作用域结束时释放栈帧中的所有stack，如果某个stack对heap中某个数据拥有所有权，那么也会释放heap中的这个数据。
## 1.1 rust中的内存释放机制
...
这样产生的问题：如果一个作用域A中的指针变量A1赋值给另一个作用域B中的变量B1，当B执行完毕会释放B1的stack及其heap，那么回到A执行时，变量A1就变成了指向不存在数据的地址的悬垂指针。如果继续使用A1会产生无法预测的问题，就是rust中的未定义行为。（标量类型默认copy不会出现这种问题。）
...
s.insert_str(0, "foo") //相当于String::insert_str(&mut s, 0, "foo")
```
### 3.3 解引用
使用`*`可以获取到引用变量指向的变量：
```rust
