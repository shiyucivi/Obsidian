...
tokio默认创建的线程是OS线程。我们可以使用`tokio::spawn`将异步任务调度到tokio的线程池中并发执行：
```rust
async run() {
	for i in 1..10 {
	    println!("{i}");
...
```
Tokio的默认工作线程池（worker threads）是为 **非阻塞、协作式异步任务** 设计的。一个spawn执行的future是一个轻量级的协程，运行时为每个协程分配一个线程。由运行时来控制线程在协程之间的工作切换。
### 3 异步任务
