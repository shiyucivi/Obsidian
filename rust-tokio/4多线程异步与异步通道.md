...
```
channel需要设置通道缓冲区大小，如果缓冲区存在空间，那么send方法会立即完成（Future的状态置为`Poll::Ready`），如果缓冲区已满，那么会让出当前线程。如果通道存在消息，那么接收端的revc方法会立即完成，如果没有消息也会让出现场。
channel返回的Sender满足Send+Sync trait。可以在多线程之间克隆并发送。但是Reciver只满足了Send，只能在单一线程中使用或转移。
可以通过Arc包装来在多个任务或线程之间使用。
#### 2.2 `sync::broadcast`
...
```
#### 2.3 `sync::oneshot`
oneshot通道和mpsc类似，但是没有缓冲区，一次只发送一条消息。
```rust
...
thread::spawn(move || {
	...
	handle.spawn(async move { 
		println!("Running in main Tokio runtime"); 
		let _ = tx.send("done");  //send不需要await
	}); 
}); 
rx.await.unwrap(); //不需要recv方法
```
注意oneshot的Sender发送是同步的不需要await，接收端本身就是一个Future不需要再调用recv方法直接await。
#### 2.4 `sync::watch`
